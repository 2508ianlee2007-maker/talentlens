...
# Updated to support MULTIPLE job descriptions (PDF + DOCX)
# ============================================================
...
# ── SECTION 5: Extract zip file (always fresh) ──
print("Current working directory:", os.getcwd())
...
jd_files  = [f for f in all_files if f.suffix.lower() in (".pdf", ".docx")]

print("CV/Resume files found:")
for f in txt_files:
    print("-", f.name)

print("\nJob Description files found (PDF + DOCX):")
for f in jd_files:
    print("-", f.name)

if len(jd_files) == 0:
    raise FileNotFoundError("No job description files found (PDF or DOCX).")

print(f"\nTotal job descriptions: {len(jd_files)}")
...
# ── SECTION 8: Read all job descriptions (PDF + DOCX) ──
...
primary_jd_name = jd_files[0].name
...
# ── SECTION 10: Save cleaned text files (always fresh) ──
...
# ── SECTION 20: Always rebuild FAISS index fresh ──
