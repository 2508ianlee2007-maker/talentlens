...
with tab_chat:
    ...
    if not st.session_state.groq_key:
        ...
        # Build context string from all loaded data
        def build_chat_context() -> str:
            ctx = ""
            if st.session_state.raw_jd_texts:
                ctx += "=== JOB DESCRIPTIONS ===\n"
                for n, t in st.session_state.raw_jd_texts.items():
                    ctx += f"\n--- {n} ---\n{t[:2500]}\n"
            if st.session_state.raw_cv_texts:
                ctx += "\n=== CANDIDATE CVs ===\n"
                for n, t in st.session_state.raw_cv_texts.items():
                    ctx += f"\n--- {n} ---\n{t[:2500]}\n"
            if st.session_state.results:
                ...
                for jd_n, res in st.session_state.results.items():
                    ...
                    for r in res:
                        ctx += (
                            f"  {r['cv_name']}: {r['score']}/10 "
                            f"({verdict_label(r['score'])[0]}) — "
                            f"{(r['summary'] or '')[:150]}\n"
                        )
            return ctx

        # ── Quick prompt buttons (shown when chat is empty) ──
            ...
            with st.chat_message("assistant"):
                with st.spinner("Thinking…"):
                    ...
                    sys_content = (
                        ...
                        "Be specific — cite candidate names and scores where relevant.\n\n"
                        + build_chat_context()
                    )
